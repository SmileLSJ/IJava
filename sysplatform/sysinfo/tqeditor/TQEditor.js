...
TQE=function(objId,userConfig)
{
	...
	$.init=function(){
		...
		if(true == $.config.hideToolBar)
			code = '<div class="" id="'+$.objId+'_EditorToolBar" style="display:none;">';
	...
	$._initContent=function(code){
		if('code'!==$.currentMode()){
			...
			if(true==$.config.readonly){
				s+='</head><body contenteditable="false" ></body></html>';//  '+ code+'
			}
			else{
...
TQE.config={
	...
	'showToolbarGroupHandle':false, //显示工具组最前的图
